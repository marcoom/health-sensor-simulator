...
class TestDotEnvFileLoading:
    ...
    def test_env_file_loading_with_mock(self):
        """Test that .env file values are loaded when environment variables are not set."""
        # Mock .env file content
        env_content = """LOG_LEVEL=DEBUG
ANOMALY_DETECTION_METHOD=EIF
EIF_THRESHOLD=0.7
DISTANCE_THRESHOLD=4.5
ALARM_ENDPOINT_URL=http://localhost:9000/alerts
"""
        
        with patch('builtins.open', mock_open(read_data=env_content)):
            with patch('os.path.exists', return_value=True):
                with patch.dict(os.environ, {}, clear=True):  # Clear environment
                    with patch('src.app.config.load_dotenv') as mock_load_dotenv:
                        # Mock load_dotenv to simulate setting environment variables
                        def side_effect():
                            os.environ.update({
                                'LOG_LEVEL': 'DEBUG',
                                'ANOMALY_DETECTION_METHOD': 'EIF', 
                        ...
                        assert settings.LOG_LEVEL == "DEBUG"
                        assert settings.ANOMALY_DETECTION_METHOD == "EIF"
                        assert settings.EIF_THRESHOLD == 0.7
                        assert settings.DISTANCE_THRESHOLD == 4.5
                        assert settings.ALARM_ENDPOINT_URL == "http://localhost:9000/alerts"
    
    def test_environment_precedence_over_env_file(self):
        """Test that environment variables take precedence over .env file."""
        # Mock .env file with one set of values
...
"""
        ...
        with patch('builtins.open', mock_open(read_data=env_content)):
            with patch('os.path.exists', return_value=True):
                ...
                with patch.dict(os.environ, {"LOG_LEVEL": "DEBUG", "ANOMALY_DETECTION_METHOD": "EIF"}):
                    settings = Settings()
                    # Environment variables should win
                    assert settings.LOG_LEVEL == "DEBUG"
                    assert settings.ANOMALY_DETECTION_METHOD == "EIF"
    
    def test_env_file_not_available(self):
        """Test behavior when .env file is not available."""
        with patch('src.app.config.load_dotenv', side_effect=ImportError):
            with patch.dict(os.environ, {}, clear=True):
                # Disable .env file loading to test true defaults
                with patch('src.app.config.Settings.Config.env_file', None):
                    settings = Settings()
                    # Should use defaults when no .env file and no environment variables
    ...
    def test_alarm_endpoint_url_init_override(self):
        """Test that __init__ method properly handles ALARM_ENDPOINT_URL."""
        # Test when ALARM_ENDPOINT_URL is not set initially but exists in environment
        with patch.dict(os.environ, {"ALARM_ENDPOINT_URL": "http://custom.example.com/alerts"}, clear=True):
            settings = Settings()
            # The __init__ method should set it from environment
            assert settings.ALARM_ENDPOINT_URL == "http://custom.example.com/alerts"
        
        # Test when ALARM_ENDPOINT_URL is already set (should not override)
        with patch.dict(os.environ, {"ALARM_ENDPOINT_URL": "http://env.example.com/alerts"}, clear=True):
            settings = Settings(ALARM_ENDPOINT_URL="http://explicit.example.com/alerts")
            # Explicit value should be preserved
            assert settings.ALARM_ENDPOINT_URL == "http://explicit.example.com/alerts"
